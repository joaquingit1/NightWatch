...
export interface IntakeResponse {
  ...
  bound_to_robot?: boolean;
  // True when the operator's AUTO ESCORT switch was on and the robot took this
  // (unbound) public response straight to a rest area.
  auto_escort_dispatched?: boolean;
  duplicate?: boolean;
}
...
export async function acknowledgeIntake(responseId: string): Promise<void> {
  ...
  if (!res.ok) {
    const detail = await res.text();
    throw new Error(detail || "robot dispatch failed");
  }
}
