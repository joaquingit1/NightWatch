...
export function IntakeFormContent() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setInteractionId(schema.interaction_id);
  ...
  const submitForm = useCallback(
    async (finalAnswers: Record<string, boolean | string>, nextOutcome: Outcome) => {
      setStep("submitting");
      try {
        // Refresh just before submission: the QR/NFC URL is fixed, while the
        // robot interaction is short-lived and may have started after the page
        // was opened. The server still accepts only the first matching answer.
        const currentSchema = await fetchIntakeSchema(sessionId);
        const response = await submitIntake({
          session_id: sessionId,
          interaction_id: currentSchema.interaction_id ?? interactionId,
          // Kept only for backwards-compatible storage. UI-design deliberately
          // contains no consent question; routing uses the two visible answers.
  ...
  const handleSelect = (value: boolean | string) => {
    ...
    window.setTimeout(() => {
      if (currentQuestion.id === "tiredness" && value === "energized") {
        void submitForm(nextAnswers, "coding");
        return;
      }
      if (currentQuestion.id === "wants_escort") {
        void submitForm(nextAnswers, value === true ? "escort" : "rest");
        return;
      }
  ...
  const escortCopy =
    result?.interaction_status === "already_handled_or_expired"
      ? {
          title: "本次引导已由其他请求处理",
          subtitle:
            "Your response was saved, but this robot interaction has already ended.",
        }
      : result?.interaction_bound
        ? {
            title: "好的，耶耶将带你去睡眠空间",
            subtitle: "All right, Yeye will guide you to the sleep space.",
          }
        : {
            title: "护送请求已登记",
            subtitle:
              "Your request was saved. Please wait for the operator to confirm.",
          };
          ...
          {step === "done" && outcome && (
            <div className="intake-state intake-state--done">
              ...
              {outcome === "coding" ? (
                ...
                <>
                  <h2>{escortCopy.title}</h2>
                  <p>{escortCopy.subtitle}</p>
                </>
              ) : (
                <>
                  <h2>好的，耶耶提醒你注意休息哦～</h2>
