...
class LivePolicyEventSource(PolicyEventSource):
    """Emit policy thoughts from live fatigue frames and intake escort queue."""
    ...
    def tick(self) -> None:
        ...
        if pending:
            ...
            self._emit(
                "ESCORT",
                f"护送 {alias} 前往休息区 | Escorting {alias} to nap zone",
                guest.session_id,
            )
            return
        ...
        if frame.calib_state in {"uncalibrated", "quick"}:
            self._emit(
                "DIAGNOSE",
                f"校准生物信号中 · RestScore {frame.score:.0f} | Calibrating biometrics",
                target,
            )
            return
        ...
        if frame.score >= self._fatigue_threshold:
            self._emit(
                "TRIAGE",
                f"RestScore {frame.score:.0f}，疲劳信号上升 | Fatigue rising, preparing triage",
                target,
            )
            return
...
# Kept for offline development only.
