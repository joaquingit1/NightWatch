    ...
    while True:
        try:
            async with connect(
                ...
                # Point clouds are already dense binary float buffers. The
                # default per-message DEFLATE negotiation spent a full CPU
                # core decompressing the robot stream and recompressing it
                # for the browser, delaying unrelated camera/API traffic.
                compression=None,
            ) as upstream:
                backoff = RECONNECT_INITIAL_S
