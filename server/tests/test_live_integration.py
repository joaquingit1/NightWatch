...
def test_live_result_preserves_all_model_tracks_and_selects_usable_primary() -> None:
    ...
    assert frame.looking_at_camera is True
    assert frame.head_yaw == -4.0
    assert frame.gaze_horizontal == 0.1
    assert frame.frame_width == 960
    assert frame.frame_height == 540
