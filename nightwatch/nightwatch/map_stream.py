...
class MapStreamer(Module):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self._latest_loaded_map: PointCloud2 | None = None
        self._seq = 0
        self._premap_seq = 0
        self._scan_seq = 0
    ...
    async def _serve(self) -> None:
        ...
        async with ws_server.serve(
            ...
            # Binary float32 point clouds gain little from DEFLATE while the
            # compression work competes directly with navigation/perception.
            # The loopback FastAPI relay forwards these snapshots verbatim.
            compression=None,
            logger=ws_logger,
        ):
    ...
    async def handle_loaded_map(self, msg: PointCloud2) -> None:
        ...
        """
        self._latest_loaded_map = msg
        points = msg.points_f32()
        aligned = False
        transform = self._reloc_transform
        # Keep refreshing even after a transform is known: an alignment can be
        # corrected mid-session (a restored transform overturned by live
        # verification, an operator initial pose), and the browser must not
        # keep converting drawn zones with the stale value. The query is
        # throttled and runs off-loop.
        ...
        status: dict[str, Any] = {"type": "premap", "aligned": aligned}
        if transform is not None:
            status["world_to_map"] = transform
        self._latest_premap_status = json.dumps(status)
    ...
    def _schedule_relocalization_query(self) -> None:
        ...
        now = time.monotonic()
        if reloc is None or now < getattr(self, "_reloc_query_not_before", 0.0):
            return
        lock = getattr(self, "_reloc_query_lock", None)
        if lock is None:
            # Unit tests build bare streamers without the query plumbing.
            return
        with lock:
    ...
    def _query_relocalization_transform(self) -> None:
        try:
            ...
            if transform is not None:
                ...
                # Rebroadcast the projected premap only when the transform
                # actually changed (first lock or a mid-session correction);
                # a steady alignment must not re-send the full cloud on every
                # throttled refresh.
                changed = value != self._reloc_transform
                self._reloc_transform = value
                loaded_map = self._latest_loaded_map
                if (
                    changed
                    and loaded_map is not None
                    and self._loop is not None
                    and not self._loop.is_closed()
                ):
                    asyncio.run_coroutine_threadsafe(
                        self.handle_loaded_map(loaded_map), self._loop
                    )
        except Exception:
