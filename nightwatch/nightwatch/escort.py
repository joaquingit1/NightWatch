...
def run_escort(
    *,
    ...
    - The nav flag is latched by the planner and only resets when the planner
      ingests the new goal on its own worker, so a finished patrol leg leaves it
      True. It is therefore trusted only after it has been observed False at
      least once since the latest ``send_goal`` (a real reset edge). The pose
      radius check needs no such guard: it is grounded in live odometry.
    ...
    try:
        ...
        while (
            ...
            goal_attempts += 1
            send_goal(gx, gy)
            # The planner's goal-reached flag stays latched from any previous
            # goal until the planner worker ingests this one, so a True read
            # straight after send_goal proves nothing. Trust it only once a
            # False has been observed for THIS goal.
            nav_flag_reset_seen = False
            if goal_attempts == 1:
            ...
            while now() < goal_deadline:
                ...
                nav_reached = arrived()
                if not nav_reached:
                    nav_flag_reset_seen = True
                if (nav_reached and nav_flag_reset_seen) or dist <= arrival_radius_m:
                    arrived_flag = True
                    break
                sleep(tick_s)
