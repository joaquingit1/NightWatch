...
class NightwatchPersonFollow(PersonFollowSkillContainer):
    ...
    def observe_person(self, min_height_frac: float = 0.0) -> dict[str, Any] | None:
        ...
        candidates = [
            d
            for d in detections
            if (d.bbox[3] - d.bbox[1]) / float(image.height) >= min_height_frac
        ]
        if not candidates:
            return None
        ...
        return {
            "bbox": [x1, y1, x2, y2],
            "offset": ((x1 + x2) / 2.0 - image.width / 2.0) / (image.width / 2.0),
            "height_frac": (y2 - y1) / float(image.height),
            "track_id": int(best.track_id),
            "frame_ts": float(image.ts or 0.0),
            "person_id": (
                getattr(self, "_person_memory", None).session_person(
                    int(best.track_id)
                )
                if getattr(self, "_person_memory", None) is not None
                else None
            ),
        }
    ...
    def _follow_loop(self, tracker: Any, query: str) -> None:
        ...
        while not self._should_stop.wait(period):
            ...
            if memory is not None and now - last_identity_at >= 4.0:
