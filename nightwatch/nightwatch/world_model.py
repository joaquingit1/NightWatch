...
# Shared operator zone file (same contract as nightwatch/navigation.py). The
# booth /lidar page writes polygons here; kind "sleeping" declares an escort
# destination rather than a movement constraint, and the world model turns
# each active sleeping polygon into one navigable `areas` row at its centroid.
_OPERATOR_ZONE_PATH = os.getenv(
    "NIGHTWATCH_ZONE_STATE_PATH", "assets/output/maps/nightwatch_zones.json"
)
# Session odometry-epoch file (owned by nightwatch/navigation.py). Zone
# world_points snapshots carry a `world_epoch` stamp; a snapshot is only
# trusted while its stamp matches the epoch recorded here.
_ODOM_EPOCH_PATH = os.getenv(
    "NIGHTWATCH_ODOM_EPOCH_PATH", "assets/output/maps/nightwatch_odom_epoch.json"
)
# areas.auto_tag prefix that links a zone-derived area back to its polygon so
# re-saves update in place and deleted polygons retract their area.
_ZONE_AREA_TAG_PREFIX = "map_zone:"


def _current_odom_epoch_id() -> str | None:
    """Return the running session's odometry epoch id, or None if unknown."""
    try:
        with open(_ODOM_EPOCH_PATH, encoding="utf-8") as handle:
            return str(json.load(handle)["epoch_id"])
...
def _polygon_centroid(points: list[tuple[float, float]]) -> tuple[float, float]:
    """Shoelace centroid, with the vertex mean for degenerate polygons.

    Module-level on purpose: the Module skill wrapper rebinds class callables,
    which breaks staticmethod semantics.
    """
    area_sum = 0.0
    cx = 0.0
    cy = 0.0
...
def _area_type_from_name(name: str) -> str:
    """Map a human label onto a canonical area type, with synonyms.

    Exact _AREA_TYPES substrings win; otherwise common ways operators say
    "sleeping area" ("nap zone", "rest spot", "bed", 休息区…) still land on
    sleeping_area instead of silently becoming an unfindable "unknown".
    """
    ...
    if any(word in normalized for word in sleeping_synonyms):
        return "sleeping_area"
    return "unknown"
...
class NightwatchWorldModel(Module):
        ...
        for table in ("areas", "objects"):
            for column in ("map_x", "map_y"):
                try:
                    ...
                    pass
        # Operator-authored rows (tag_area_here, map-drawn sleeping zones) are
        # protected from vote-based type reversion; see _resolve_area.
        try:
            self._db.execute(
                "ALTER TABLE areas ADD COLUMN protected INTEGER NOT NULL DEFAULT 0"
            )
        except sqlite3.OperationalError:
            pass
        self._db.commit()
    ...
    def _on_marker_detections(self, msg: Detection3DArray) -> None:
        ...
        for det in msg.detections[: msg.detections_length]:
            ...
            if name and evidence == self.config.marker_min_evidence:
                ...
                # The spatial-memory tag alone is invisible to the escort:
                # nearest_area() reads ONLY the areas table. A named marker is
                # an explicit physical declaration, so it must also become a
                # navigable area at the safe approach pose (BRIDGE.md promises
                # exactly this for AprilTag id 2 = sleeping_area).
                area_type = _area_type_from_name(name)
                if area_type != "unknown" and db is not None:
                    with self._lock:
                        area_id = self._ensure_area(target_x, target_y, now)
                        db.execute(
                            """
                            UPDATE areas SET area_type=?,confidence=1.0,
                              auto_tag=?,last_seen=?,protected=1
                            WHERE area_id=?
                            """,
                            (
                                area_type,
                                f"marker:{marker_id}:{name}",
                                now,
                                area_id,
                            ),
                        )
                        db.commit()
                    logger.info(
                        "marker registered as navigable area",
                        marker_id=marker_id,
                        area_type=area_type,
                        area_id=area_id,
        ...
        with self._lock:
            row = db.execute(
                ...
                SELECT area_id,center_x,center_y,samples,area_type,confidence,
                       auto_tag,first_seen,last_seen,protected
    ...
    def _resolve_area(self, area_id: str) -> None:
        ...
        with self._lock:
            ...
            if area is None:
                return
            if int(area.get("protected") or 0):
                # An operator said so: a human tag (tag_area_here, MARK SLEEP
                # button, map-drawn sleeping zone, named AprilTag) is never
                # vote-driven. Evidence keeps accumulating, but resolution
                # leaves the row's type, confidence, and tag untouched.
                return
    ...
    def _merge_pair_locked(self, area_id_a: str, area_id_b: str) -> str | None:
        ...
        a_protected = int(a.get("protected") or 0)
        b_protected = int(b.get("protected") or 0)
        if a_protected != b_protected:
            # An operator-authored area always survives a merge so its type
            # and identity cannot be absorbed by a larger auto-detected room.
            survivor, loser = (a, b) if a_protected else (b, a)
        elif b_samples > a_samples or (
    ...
    def _watchdog_loop(self) -> None:
        ...
        while not self._stop_event.wait(1.0):
            ...
            if ticks % 5 == 2:
                try:
                    self._sync_sleeping_zones()
                except Exception:
                    logger.exception("sleeping zone sync failed")
    ...
    def world_status(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            sleeping_areas = int(
                db.execute(
                    "SELECT COUNT(*) FROM areas WHERE area_type='sleeping_area'"
                ).fetchone()[0]
            )
            sleeping_rows = db.execute(
                """
                SELECT area_id,center_x,center_y,map_x,map_y,auto_tag,protected
                FROM areas WHERE area_type='sleeping_area'
                """
            ).fetchall()
            keyframes = (
        ...
        try:
            ...
            coverage = {}
        # Per-area coordinates so the booth map can render markers, not only a
        # count. Current-session rows use their live world centers; anchored
        # prior-session rows are re-projected map->world through the current
        # transform; unanchored prior-session rows report their stale center
        # with anchored=False so the UI can display them as tentative.
        transform = self._world_to_map()
        session_ids = set(self._session_area_ids)
        sleeping_area_list: list[dict[str, Any]] = []
        for area_id, cx, cy, map_x, map_y, auto_tag, protected in sleeping_rows:
            anchored = map_x is not None and map_y is not None
            x, y = float(cx), float(cy)
            if (
                str(area_id) not in session_ids
                and anchored
            ...
            sleeping_area_list.append(
                {
                    ...
                    "label": str(auto_tag or "sleeping_area"),
                }
            )
        return {
            "enabled": True,
            ...
            "markers": markers,
            "sleeping_areas": sleeping_areas,
            "sleeping_area_list": sleeping_area_list,
    ...
    @skill
    def _sync_sleeping_zones(self) -> None:
        """Turn operator-drawn "sleeping" polygons into navigable areas.

        The /lidar page saves polygons into the shared zone file. keep_in and
        keep_out constrain navigation and are consumed by the planners;
        "sleeping" declares where a mattress is. Every active sleeping polygon
        is upserted as one `areas` row (area_type='sleeping_area', confidence
        1.0) at its centroid in the CURRENT world frame, so the escort's
        nearest_area() finds it exactly like a self-tagged or button-tagged
        area. Rows are keyed to their polygon through auto_tag, so re-saving a
        polygon updates the same row and deleting or deactivating it retracts
        the area. Re-syncs when the file changes or when the world->map
        transform first becomes available (map-frame polygons project better
        once relocalization locks).
        """
        db = self._db
        if db is None:
            return
        path = _OPERATOR_ZONE_PATH
        try:
            mtime = os.stat(path).st_mtime_ns
        except OSError:
            mtime = None
        transform = self._world_to_map()
        # Keyed on the transform VALUE, not merely its presence: a corrected
        # lock (restored alignment overturned by live verification, operator
        # initial pose) must re-project every zone-derived area even though
        # the zone file itself did not change.
        sync_key = (mtime, transform)
        if sync_key == getattr(self, "_sleeping_zone_sync_key", None):
            return
        self._sleeping_zone_sync_key = sync_key
        current_epoch = _current_odom_epoch_id()
        ...
        if mtime is not None:
            try:
                ...
                logger.exception("sleeping zone file unreadable", path=path)
                return

        def world_centroid(item: dict[str, Any]) -> tuple[float, float] | None:
            try:
                map_points = [
            ...
            # Same legacy inference as navigation._load_operator_zones: an
            # entry without a frame whose two polygons are identical is a
            # pre-anchor WORLD drawing, not MAP coordinates.
            legacy_world = (
                not frame
                and len(map_points) == len(world_points)
                and map_points == world_points
            )
            world_stamp = item.get("world_epoch")
            world_stamp = str(world_stamp) if world_stamp else None

            def snapshot_is_current() -> bool:
                # An unstamped snapshot keeps legacy behavior; a stamped one
                # must belong to this session's odometry frame, otherwise the
                # centroid would land where the polygon was LAST session
                # (AGENTS invariant 10: refuse, do not guess).
                return world_stamp is None or world_stamp == current_epoch

            if (frame == "world" or legacy_world) and len(world_points) >= 3:
                if not snapshot_is_current():
                    return None
                return _polygon_centroid(world_points)
            # Map-frame polygon: prefer projecting through the live transform;
            # fall back to the stored world snapshot while unlocked.
            if transform is not None and len(map_points) >= 3:
                tx, ty, yaw = transform
                cos_yaw = math.cos(yaw)
                sin_yaw = math.sin(yaw)
                unprojected = [
                    (
                        cos_yaw * (mx - tx) + sin_yaw * (my - ty),
                        -sin_yaw * (mx - tx) + cos_yaw * (my - ty),
                    )
                    for mx, my in map_points
                ]
                return _polygon_centroid(unprojected)
        ...
        for item in zones:
            ...
            if not zone_id:
                continue
            centroid = world_centroid(item)
            if centroid is not None:
                active[zone_id] = centroid

        now = time.time()
        with self._lock:
            existing: dict[str, str] = {}
            for area_id, auto_tag in db.execute(
                "SELECT area_id,auto_tag FROM areas WHERE auto_tag LIKE ?",
                (f"{_ZONE_AREA_TAG_PREFIX}%",),
            ).fetchall():
                existing[str(auto_tag)[len(_ZONE_AREA_TAG_PREFIX) :]] = str(area_id)

            for zone_id, (x, y) in active.items():
                area_id = existing.get(zone_id)
                if area_id is None:
                    area_id = f"area_{uuid.uuid4().hex[:10]}"
                    db.execute(
                        """
                        INSERT INTO areas
                        (area_id,center_x,center_y,samples,area_type,confidence,
                         auto_tag,first_seen,last_seen,protected)
                        VALUES(?,?,?,?,?,?,?,?,?,1)
                        """,
                        (
                            ...
                            "sleeping_area",
                            1.0,
                            f"{_ZONE_AREA_TAG_PREFIX}{zone_id}",
                            now,
                            now,
                        ),
                    )
                    logger.info(
                        "sleeping zone registered as area",
                        zone_id=zone_id,
                        area_id=area_id,
                    ...
                    db.execute(
                        """
                        UPDATE areas SET center_x=?,center_y=?,
                          area_type='sleeping_area',confidence=1.0,last_seen=?,
                          protected=1
                        WHERE area_id=?
                        """,
                        (x, y, now, area_id),
                    )
                # Session membership makes nearest_area use the fresh world
                # center and lets _backfill_map_coords stamp map coords.
                self._session_area_ids.add(area_id)

            for zone_id, area_id in existing.items():
                if zone_id in active:
                    continue
                db.execute("DELETE FROM areas WHERE area_id=?", (area_id,))
                db.execute("DELETE FROM area_votes WHERE area_id=?", (area_id,))
                self._session_area_ids.discard(area_id)
                logger.info(
                    "sleeping zone removed; area retracted",
                    zone_id=zone_id,
                    area_id=area_id,
                )
            db.commit()
    ...
    def tag_area_here(self, name: str) -> str:
        ...
        x = float(odom.position.x)
        y = float(odom.position.y)
        area_type = _area_type_from_name(clean)
        db = self._db
        # One lock span for ensure + type override, so a concurrent
        # _resolve_area from the semantic worker cannot interleave between the
        # row's creation and its protection (RLock: _ensure_area re-enters).
        with self._lock:
            ...
            if db is not None:
                db.execute(
                    ...
                    UPDATE areas SET area_type=?,confidence=1.0,auto_tag=?,
                      last_seen=?,protected=1 WHERE area_id=?
                    """,
