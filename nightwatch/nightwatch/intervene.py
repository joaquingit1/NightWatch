...
class InterventionSkill(Module):
    ...
    def _detect_people(self, image: Image) -> list[Any]:
        try:
            cached = self._follow.observe_people_pose(0.08)
        except Exception:
            # Compatibility for isolated deployments/tests that do not bind
            # NightwatchPersonFollow. Production always uses the shared cache.
            logger.warning(
                "shared pose cache unavailable; using local fallback",
                exc_info=True,
            )
